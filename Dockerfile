...
# Make folder
RUN mkdir -p probabilistic-poses

# Install all other python deps
COPY [ "./setup.py",  "probabilistic-poses/setup.py" ]
RUN python3 -m pip install --no-cache-dir `python3 probabilistic-poses/setup.py --list-reqs`

# Install repo
COPY [ ".",  "probabilistic-poses/" ]
RUN python3 -m pip install --no-cache-dir probabilistic-poses/