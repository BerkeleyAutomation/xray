...
class DetectionTargetLayer(KE.Layer):
    """Subsamples proposals and generates target box refinement, class_ids for each.
...
def load_image_gt(dataset, config, image_id, augment=False, augmentation=None):
    ...
    dist = dataset.load_dist(image_id)
    ...
    dist, window, scale, padding, crop = utils.resize_image(
        dist,
        min_dim=config.IMAGE_MIN_DIM,
        min_scale=config.IMAGE_MIN_SCALE,
        max_dim=config.IMAGE_MAX_DIM,
    ...
    if augmentation:
        ...
        dist = det.augment_image(dist.astype(np.uint8),
                                 hooks=imgaug.HooksImages(activator=hook))
        # Verify that shapes didn't change
        assert image.shape == image_shape, "Augmentation shouldn't change image size"
        assert dist.shape == dist_shape, "Augmentation shouldn't change dist size"
        # Change mask back to bool
        dist = dist.astype(np.bool)

    # Bounding boxes. Note that some boxes might be all zeros
    # if the corresponding mask got cropped out.
    # bbox: [num_instances, (y1, x1, y2, x2)]
    bbox, class_ids = utils.extract_bboxes(dist)
...
class MaskRCNN():
    ...
    def build(self, mode, config):
        ...
        if mode == "training":
            ...
            input_gt_dist = KL.Input(
                shape=[2,None], name="input_gt_dist", dtype=tf.int32
            )
